# codigo responsavel pelo primeiro nó de cliente no chat p2p
# Autor: Allan
    ...
    while True:
        try:
            ...
            if tipo == TIPO_NORMAL:
                ...
                print(f"[ECHO de {apelido}]: {mensagem} (active user)")
...
def main():
    ...
    porta_local = int(input("enter local port to listen on (e.g.: 5001): "))
    
    ip_destino = input("enter destination IP (e.g.: 127.0.0.1): ").strip()
    porta_destino = int(input("enter destination port (e.g.: 5002): "))
